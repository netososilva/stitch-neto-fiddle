...
async function gerarEPUB(){
 ...
 try{
  ...
  const css=`body{font-family:sans-serif;line-height:1.45;margin:5%;color:#111}h1,h2{text-align:center}.capa,.metadados{text-align:center}.metadados{margin:1.5em 0}.progresso-ascii{font-family:monospace;white-space:pre;text-align:center;margin:1em 0}.receita{text-align:center;margin:2em 0}.instrucao{display:inline;font-weight:bold}.bloco{display:inline-block;padding:.25em .45em;margin:.12em;border:1px solid #222;font-weight:bold;border-radius:.2em}nav ol{padding-left:1.4em}`;
  ...
  function adicionarCapitulo(indice,itens,ordem,nome){
   ...
   const larguraBarra=20, preenchidos=Math.round(percentual*larguraBarra/100);
   const barraAscii=`[${"#".repeat(preenchidos)}${"-".repeat(larguraBarra-preenchidos)}] ${percentual.toFixed(1)}%`;
   const corpo=`<section epub:type="chapter"><h1>${epubEscapar(titulo)}</h1><p class="progresso-ascii" aria-label="Progresso acumulado: ${feitos} de ${quadrados} quadrados (${percentual.toFixed(1)}%)">${barraAscii}</p><div class="receita">${instrucoes}</div></section>`;
...
.progresso-ascii { font-family: monospace; white-space: pre; text-align: center; margin: 1em 0; }
