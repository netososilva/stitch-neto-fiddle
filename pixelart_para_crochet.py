...
def abrir_seletor_de_projetos():
    ...
    tk.Button(painel, text="Escolher imagem", command=escolher_arquivo, bg="#8b5fbf", activebackground="#b794f4", fg="white", activeforeground="white", relief="flat", padx=34, pady=12, width=24, font=("Segoe UI", 11, "bold")).pack()
...
/* Mantém receitas por extenso dentro da largura disponível, inclusive no Zen. */
.linhaAtualReceita > span{
    min-width:0;
...
@media (max-width:600px){
    ...
    /* A legenda permanece legível sem extrapolar a viewport. */
    #legenda{
        width:100%;
        table-layout:fixed;
    }
    ...
    /* No Zen, o painel precisa usar a largura real do celular para os itens quebrarem. */
    body.focus-mode #painel_c2c,
    body.focus-mode.layout-left #painel_c2c,
    body.focus-mode.layout-center #painel_c2c,
    body.focus-mode.layout-right #painel_c2c{
...
html.append('<div id="alertaLimite" role="status" aria-live="polite" style="display:none;margin:8px auto 14px;max-width:620px;padding:9px 12px;border:1px solid #c99b3b;border-radius:8px;background:#fff7df;color:#6b4b08;text-align:center;font-size:13px;font-weight:600"></div>')
...
html.append('<button onclick="gerarEPUBCompativel()" id="btnEPUBCompativel">Gerar EPUB — Compatibilidade</button>')
...
def imagem_epub_nitida(imagem, lado_maximo=1600):
    """Amplia pixels pequenos sem suavizá-los para a tela do Kindle."""
    resultado = imagem.copy()
    maior_lado = max(resultado.size)
    if maior_lado < lado_maximo:
        escala = (lado_maximo + maior_lado - 1) // maior_lado
        resultado = resultado.resize(
            (resultado.width * escala, resultado.height * escala), reamostragem_epub
        )
    elif maior_lado > lado_maximo:
        resultado.thumbnail((lado_maximo, lado_maximo), reamostragem_epub)
    return resultado
...
html.append(f"const epubLargura={largura};const epubAltura={altura};const epubTotalCores={len(cores)};const larguraReceita={largura};const alturaReceita={altura};")
html.append('''

function epubEscapar(valor){
 // Alguns leitores Xteink não interpretam &apos; corretamente. A referência
 // numérica é equivalente e funciona também nos renderizadores mais antigos.
...
let epubParte=0;
const epubMaxPartesCompatibilidade=8;
async function gerarEPUB(){
 ...
 try{
  ...
  const totalPartes=epubParte?Math.min(epubMaxPartesCompatibilidade,total):1;
  const linhasPorParte=Math.ceil(total/totalPartes);
  ...
  function adicionarPaginas(indice,itens,ordem,prefixo){
   ...
   paginas.forEach((estado)=>{
    ...
    const concluida=indiceItemAtual=>indiceItemAtual<indiceItem || (indiceItemAtual===indiceItem && final);
    // A edição compatível usa <del>, que leitores antigos reconhecem melhor
    // que text-decoration aplicado a um span.
    const riscar=(conteudo,estaConcluida)=>estaConcluida
     ? (epubCompatibilidade?`<del class="concluida">${conteudo}</del>`:`<span class="concluida">${conteudo}</span>`)
     : conteudo;
    const textoInstrucao=(conteudo,estaConcluida)=>epubCompatibilidade && estaConcluida?`[FEITO] ${conteudo}`:conteudo;
    const instrucoes=usarNome
     ? itens.map((item,indiceItemAtual)=>{const feita=concluida(indiceItemAtual);return riscar(`<span class="instrucao">${textoInstrucao(`(${item.qtd}) ${epubEscapar(item.nome)}`,feita)}</span>`,feita);}).join(", ")
     : itens.map((item,indiceItemAtual)=>{const feita=concluida(indiceItemAtual);return riscar(`<span class="bloco" style="background-color:${item.bg};color:${item.fg}">${textoInstrucao(`${epubEscapar(item.codigo)}×${item.qtd}`,feita)}</span>`,feita);}).join("");
    ...
    const barraCompativel=epubCompatibilidade
     ? `<table class="progresso" role="presentation" aria-label="Progresso acumulado: ${percentual.toFixed(1)}%"><tr><td class="progresso-preenchimento" width="${percentual.toFixed(1)}%" bgcolor="#4caf50" style="width:${percentual.toFixed(1)}%;background:#4caf50">&#160;</td><td class="progresso-restante" width="${(100-percentual).toFixed(1)}%" bgcolor="#eeeeee" style="width:${(100-percentual).toFixed(1)}%;background:#eeeeee">&#160;</td></tr></table>`
     : `<table class="progresso" role="presentation" aria-label="Progresso acumulado: ${percentual.toFixed(1)}%"><tr><td class="progresso-preenchimento" style="width:${percentual.toFixed(1)}%">&#160;</td><td class="progresso-restante" style="width:${100-percentual.toFixed(1)}%">&#160;</td></tr></table>`;
    const segmentos=20, preenchidos=Math.round(percentual*segmentos/100);
    const barraTexto=epubCompatibilidade?`<p><strong>[${"#".repeat(preenchidos)}${"-".repeat(segmentos-preenchidos)}] ${percentual.toFixed(1)}%</strong></p>`:"";
    const corpo=`<section id="${ancora}" class="${etapaClasse}"${etapaEstilo} epub:type="chapter"><h1>${epubEscapar(tituloBase)}</h1>${exibirAlerta?`<p class="alerta-limite">${epubEscapar(alertaLimite)}</p>`:''}${barraTexto}${barraCompativel}<p class="progresso-texto"><strong>Progresso acumulado:</strong> ${feitos} de ${quadrados} quadrados (${percentual.toFixed(1)}%)</p><p style="margin-top:1em"><strong>Instruções concluídas:</strong> ${indiceItem<0?0:indiceItem+(final?1:0)} de ${totalInstrucoes}</p><div class="receita">${instrucoes}</div>${contadorHtml}</section>`;
  ...
  const nomeDownload=nomeBase+(epubParte?` - Parte ${epubParte} de ${totalPartes}`:"");
...
async function gerarEPUBCompativel(){
 ...
 try{
  const totalPartes=Math.min(epubMaxPartesCompatibilidade,receita.length);
  for(epubParte=1;epubParte<=totalPartes;epubParte++) await gerarEPUB();
...
function textoLimite(linha){
 const avisos=[];
 if(linha===larguraReceita+1) avisos.push(`Limite da largura (${larguraReceita}) alcançado — não haverá aumento nessa direção.`);
 if(linha===alturaReceita+1 && alturaReceita!==larguraReceita) avisos.push(`Limite da altura (${alturaReceita}) alcançado — não haverá aumento nessa direção.`);
 return avisos.join(' ');
}
...
document.getElementById("irLinha").addEventListener("keydown",function(e){
 if(e.key==="Enter"){
  e.preventDefault();
  irPara();
 }
...
let th=localStorage.getItem("c2c_theme")||"lilac-dark";
...
.progresso-texto { text-align: center; font-weight: bold; }
...
'''.strip()
    ...
    mini_grafico = imagem_epub_nitida(img)
...
<section epub:type="chapter">
  ...
  <table class="progresso" role="presentation" aria-label="Progresso acumulado: {percentual:.1f}%"><tr>
    <td class="progresso-preenchimento" width="{percentual:.1f}%" bgcolor="#4caf50" style="width:{percentual:.1f}%;background:#4caf50">&#160;</td>
    <td class="progresso-restante" width="{100 - percentual:.1f}%" bgcolor="#eeeeee" style="width:{100 - percentual:.1f}%;background:#eeeeee">&#160;</td>
  </tr></table>
  <p class="progresso-texto"><strong>Progresso acumulado:</strong> {feitos} de {total_quadrados} quadrados ({percentual:.1f}%)</p>
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles><rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml" /></rootfiles>
    ...
    with zipfile.ZipFile(arquivo_epub, "w") as epub:
        ...
        epub.writestr("OEBPS/capa.png", capa_buffer.getvalue())
